var ckeditor_img_plugin = new Class({
	...
	createDirTree: function(dir, objects, obj){
		...
		var del = new Element('div',{'text':'del','class':'del','styles':{'display':'none','float':'right'}});
		var div = new Element('div', {'id':'cke_div_content_'+thisTemp.counter, 'class':'content cke_dir', 'styles':{'display':'block'}});
		var edit = new Element('div',{'id':'cke_edit_button_'+thisTemp.counter, 'text':'edit','class':'edit cke_dir','styles':{'display':'none','float':'right'}});
	...
	addImg: function(picture, id, thisTemp){
		...
		var src = thisTemp.options.config.server_main_dir+'/'+picture['src'];
		var type = src.substring(src.length-5,src.length).split('.');
		if(typeof(type[1]) != "undefined" && type[1].toLowerCase() == 'gif' || type[1].toLowerCase() == 'png' || type[1].toLowerCase() == 'jpeg' || type[1].toLowerCase() == 'jpg')
		{
			src ='/.temp_pic/.thumbnail/'+picture['src'];
		}
		src = src.split('//');
		src = src.join('/');
		src = thisTemp.options.config.server_temp_dir+ src;
		var img = new Element('img',{'title':picture['file'], 'alt':picture['file'], 'src':src, 'styles':{'height':picture['y'],'width':picture['x'], 'padding-left':(115-picture['x'])/2+5, 'padding-right':(115-picture['x'])/2+5, 'padding-top':(85-picture['y'])/2+5, 'padding-bottom':(85-picture['y'])/2+5}});
				...
				else
				{
					...
					if(idPanel == 0)
					{
						...
						el.addEvent('click', function(e){
							e.stop();
							//thisTemp.ckeditor.insertHtml('<img alt="" src="'+ thisTemp.options.config.server_main_dir + '/' + p["create"]+ '" />');
							$('casestudies_obj_'+objectId).set('value',thisTemp.options.config.server_main_dir + '/' + p["create"]);
					...
					else
					{
						...
						var src = thisTemp.options.config.server_main_dir+'/'+p['create'];
						var type = src.substring(src.length-5,src.length).split('.');
						if(typeof(type[1]) != "undefined" && type[1].toLowerCase() == 'gif' || type[1].toLowerCase() == 'png' || type[1].toLowerCase() == 'jpeg' || type[1].toLowerCase() == 'jpg')
						{
							src ='/.temp_pic/.thumbnail/'+p['create'];
						}
						src = src.split('//');
						src = src.join('/');
						src = thisTemp.options.config.server_temp_dir+ src;
					...
					link_del = span.getParent().getChildren('.del')[0];
					link_del.removeEvents('click');
					link_del.addEvent('click', function(e){
						e.stop();
						var answer = confirm(thisTemp.options.language.confirm);
						if(answer)
						{
							//do not use getDir metod. In search mode it don't work.
							$('cke_ajax_loader').setStyle('display','block');
							var jsonRequest = new Request.JSON({method:'post',noCache:true,url: e.target, data:{'url':p['create']}, onSuccess: function(p){
								if(p['create'] == 'true')
								{
									var parent = e.target.getParent();
									parent.empty();
		...
		$each($('documents_ajax').getElements('.image_cke'), function(el, index){
			...
			el.addEvent('click', function(e){
    			...
    			if(idPanel != 0) // is not search
    			{
    				...
    				//tempThis.ckeditor.insertHtml('<img alt="" src="'+ tempThis.options.config.server_main_dir+'/'+dir+ this.getElement('img').get('alt') +'" />');
    				$('casestudies_obj_' + objectId).set('value',tempThis.options.config.server_main_dir+'/'+dir+ this.getElement('img').get('alt'));
    			}
    			else  //is search
    			{
    				...
    				src = src.split('//');
    				src = src.join('/');
    				src = tempThis.options.config.server_main_dir + src;
    				//tempThis.ckeditor.insertHtml('<img alt="" src="'+src +'" />');
    				$('casestudies_obj_' + objectId).set('value',src);
    			}
		...
		//	progress = 'b.jpg',
			...
			onSelectSuccess: function(files) {
				...
				files.each(function(file) {
					refresh = new Element('li', {'class': 'refresh'});
					refresh.set('html', tempThis.options.language.renew).removeEvents().addEvent('click', function() {
						file.requeue();
						return false;
					});
					refresh.inject(file.ui.cancel, 'after');
				});
			},
			...
			onFileRequeue: function(file) {
				if(file.ui.element.getElement('.file-error') != null)
					file.ui.element.getElement('.file-error').destroy();
	 
				if(file.ui.cancel != null)
