...
FancyUpload3.Attach = new Class({
	...
	options: {
		queued: false,
		instantStart: true,
		bar: CKEDITOR.basePath+'plugins/image_uploader/fancy/bar.gif',
		progress: CKEDITOR.basePath+'plugins/image_uploader/fancy/progress.gif',
...
FancyUpload3.Attach.File = new Class({
	...
	render: function() {
		...
		var progress = new Element('img', {'class': 'file-progress', src: CKEDITOR.basePath+'plugins/image_uploader/fancy/progress.gif'}).inject(this.ui.size, 'after');
		this.ui.progress = new Fx.ProgressBar(progress, {
			fit: true,
			src: CKEDITOR.basePath+'plugins/image_uploader/fancy/bar.gif'
		}).set(0);
	...
	onComplete: function() {
		...
		this.response = JSON.decode(this.response.text); // Added by woj
